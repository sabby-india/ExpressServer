...
.get(cors.cors,(req,res,next) => {
    ...
    .then((dish) => {
        if(dish != null){
            ...
            err.status = 404;
            return next(err);//this willbe handles by app.js file error handler
        }        
    }, (err) => next(err))
    .catch((err) => next(err));
})
.post(cors.corsWithOptions,authenticate.verifyUser,(req,res,next) => {
    ...
    .then((dish) => {
        if(dish != null){
            req.body.author = req.user._id; //for mongoose population
            dish.comments.push(req.body);
            dish.save()
            .then((dish) => {
                Dishes.findById(dish._id)
                //mongoose population added code
                    .populate('comments.author')
                    .then((dish) => {
                        res.statusCode = 200;
                        res.setHeader('Content-Type', 'application/json');
...
.put(cors.corsWithOptions,authenticate.verifyUser, (req,res,next) => {
    ...
    .then((dish) => {
        if(dish != null && dish.comments.id(req.params.commentId) != null){
            ...
            console.log(req.user._id +'and' + dish.comments.id(req.params.commentId).author);
            ...
            if(req.user._id.equals(dish.comments.id(req.params.commentId).author)){
                ...
                err = new Error('Comment was not given by you, so you can\'t change it !');
                err.status = 403;
                return next(err);//this willbe handles by app.js file error handler
            }
    ...
    .then((dish) => {
        if(dish != null && dish.comments.id(req.params.commentId) != null ){
            ...
            if(req.user._id.equals(dish.comments.id(req.params.commentId).author)){
                ...
                err = new Error('Comment was not given by you, so you can\'t delete it !');
                err.status = 403;
                return next(err);//this willbe handles by app.js file error handler
            }
